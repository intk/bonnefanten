...
def artwork_material(obj):
    ...
    if isinstance(materials, str):
        materials_list = [material.strip() for material in materials.split(',') if material.strip()]
    # If it's already a list or a tuple (or any iterable but string), just strip the materials
        ...
        materials_list = [material.strip() for material in materials if material.strip()]
